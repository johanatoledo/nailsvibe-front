export const servicios = [
  {
    id: 1,
    nombre: "Exfoliación Pies",
    descripcion:
      "Exfoliación profunda. Masaje relajante. Hidratación intensiva. Eliminación de células muertas y suavizado de la piel de los pies.",
    precio: 25,
    duracionMinutos: 45,
    categoria: "Exfoliación",
    imagen: "/images/exfoliacionpies.png",
  },
  {
    id: 2,
    nombre: "Exfoliación Manos",
    descripcion:
      "Exfoliación profunda con parafina. Eliminación de células muertas. Masaje relajante. Hidratación intensiva. Suavizado de la piel de las manos.",
    precio: 25,
    duracionMinutos: 40,
    categoria: "Exfoliación",
    imagen: "/images/parafina.png",
  },
  {
    id: 3,
    nombre: "Manicura y pedicura",
    descripcion:
      "Manicura y pedicura completa con esmalte semipermanente. Limado. Cutículas, masaje y aplicación de esmalte duradero.",
    precio: 45,
    duracionMinutos: 120,
    categoria: "Manicura semipermanente",
    imagen: "/images/manicuraypedi.png",
  },
  {
    id: 4,
    nombre: "Uñas acrílicas",
    descripcion:
      "Preparación de uñas. Diseño personalizado. Aplicación de acrílico y decoración según tus preferencias.",
    precio: 70,
    duracionMinutos: 120,
    categoria: "Uñas acrílicas",
    imagen: "/images/unaslunares.png",
  },
  {
    id: 5,
    nombre: "Uñas de gel con diseño artístico",
    descripcion:
      "Preparación de uñas. Aplicación de gel y decoración creativa según tus gustos.",
    precio: 10,
    duracionMinutos: 90,
    categoria: "Uñas de gel",
    imagen: "/images/unascerezas.png",
  },
  {
    id: 6,
    nombre: "Uñas de gel con diseño floral",
    descripcion:
      "Preparación de uñas. Aplicación de gel. Decoración creativa con diseño floral según tus preferencias.",
    precio: 100,
    duracionMinutos: 105,
    categoria: "Nail Art",
    imagen: "/images/unasazuleslunares.png",
  },
  {
    id: 7,
    nombre: "Uñas en polygel con diseños",
    descripcion:
      "Preparación de uñas. Aplicación de polygel. Decoración creativa según tus preferencias.",
    precio: 90,
    duracionMinutos: 120,
    categoria: "Polygel",
    imagen: "/images/unasblancas.png",
  },
  {
    id: 8,
    nombre: "Uñas efecto cromo",
    descripcion:
      "Uñas con efecto cromo, que incluye preparación de uñas, aplicación de esmalte y acabado brillante y reflectante.",
    precio: 120,
    duracionMinutos: 100,
    categoria: "Uñas efecto cromo",
    imagen: "/images/unasmetalizadas.png",
  },
  {
    id: 9,
    nombre: "Uñas con diseño de flores y piedras",
    descripcion:
      "Preparación de uñas. Diseño de flores y piedras. Aplicación de esmalte y decoración creativa según tus preferencias.",
    precio: 120,
    duracionMinutos: 135,
    categoria: "Jelly Nails",
    imagen: "/images/unasflorrosada.png",
  },
  {
    id: 10,
    nombre: "Uñas matizadas",
    descripcion:
      "Preparación de uñas. Aplicación de esmalte y acabado elegante.",
    precio: 90,
    duracionMinutos: 90,
    categoria: "Polygel",
    imagen: "/images/unasrojasdorado.png",
  },
  {
    id: 11,
    nombre: "Uñas lunares TENDENCIA",
    descripcion:
      "Incluye preparación de uñas, aplicación de esmalte y decoración creativa según tus preferencias.",
    precio: 90,
    duracionMinutos: 105,
    categoria: "Uñas acrílicas",
    imagen: "/images/unaslunares.png",
  },
  {
    id: 12,
    nombre: "Manicura semipermanente",
    descripcion:
      "Preparación de uñas, aplicación de esmalte y acabado profesional y natural.",
    precio: 50,
    duracionMinutos: 60,
    categoria: "Manicura semipermanente",
    imagen: "/images/unasbasicas.png",
  },
  {
    id: 13,
    nombre: "Uñas lunares, piedras y flores",
    descripcion:
      "Preparación de uñas, aplicación de esmalte y decoración creativa según tus preferencias hasta 4 colores a escoger.",
    precio: 90,
    duracionMinutos: 120,
    categoria: "Uñas acrílicas",
    imagen: "/images/unaslunaresnegro.png",
  },
  {
    id: 14,
    nombre: "Uñas con diseños frutales y piedras",
    descripcion:
      "Más de 100 modelos de diseño. Preparación de uñas, aplicación de esmalte y decoración creativa según tus preferencias.",
    precio: 120,
    duracionMinutos: 135,
    categoria: "Jelly Nails",
    imagen: "/images/unascerezas.png",
  },
  {
    id: 15,
    nombre: "Pedicura con pedrería",
    descripcion:
      "Pedicura con pedrería en tonos vibrantes. Preparación, limpieza y exfoliación de uñas y pies. Aplicación de esmalte y decoración creativa según tus preferencias.",
    precio: 70,
    duracionMinutos: 90,
    categoria: "Pedicura",
    imagen: "/images/pedicurarojatigre.png",
  },
  {
    id: 16,
    nombre: "Pedicura sencilla",
    descripcion:
      "Preparación de uñas, limpieza y exfoliación de uñas y pies. Aplicación de suave capa de esmalte y decoración sencilla.",
    precio: 70,
    duracionMinutos: 75,
    categoria: "Pedicura",
    imagen: "/images/pedicuranaranja.png",
  },
  {
    id: 17,
    nombre: "Pedicura con decoración floral y pedrería",
    descripcion:
      "Preparación de uñas, limpieza y exfoliación de uñas y pies. Aplicación de suave capa de esmalte, decoración floral y pedrería.",
    precio: 70,
    duracionMinutos: 90,
    categoria: "Pedicura",
    imagen: "/images/pedicurablanca.png",
  },
  {
    id: 18,
    nombre: "Pedicura con diseños personalizados en tonos pasteles",
    descripcion:
      "Preparación de uñas, limpieza y exfoliación de uñas y pies. Aplicación de suave capa de esmalte en combinación con más de 20 tonos pasteles.",
    precio: 45,
    duracionMinutos: 80,
    categoria: "Pedicura",
    imagen: "/images/pedicuraamarilla.png",
  },
  {
    id: 19,
    nombre: "Pedicura en tonos blancos",
    descripcion:
      "Preparación de uñas, limpieza y exfoliación de uñas y pies. Aplicación de suave capa de esmalte y decoración floral o con pedrería.",
    precio: 45,
    duracionMinutos: 80,
    categoria: "Pedicura",
    imagen: "/images/pedicura1blanca.png",
  },
];