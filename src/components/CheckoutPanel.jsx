...
export default function CheckoutPanel({
  ...
  const itemsServicio = Array.isArray(carrito)
    ? carrito
    : [];
  ...
  const [numeroOperacion, setNumeroOperacion] =
    useState("");
  ...
  const [nombreError, setNombreError] =
    useState("");

  const [telefonoError, setTelefonoError] =
    useState("");

  const [fechaError, setFechaError] =
    useState("");

  const [horaError, setHoraError] =
    useState("");

  const [pagoError, setPagoError] =
    useState("");
  ...
  const montoReserva =
    totalServicio * PORCENTAJE_RESERVA;

  const montoRestante =
    totalServicio - montoReserva;

  const datosPago =
    DATOS_PAGO[metodoPago];
  ...
  const confirmarCita = async () => {
    ...
    const telefonoLimpio =
      clienteTelefono.replace(/\D/g, "");

    const operacionLimpia =
    ...
    if (!nombreLimpio) {
      setNombreError(
        "Ingresa tu nombre para agendar la cita."
      );
    ...
    try {
      ...
      setMensajeExito(
        "Cita registrada correctamente. Redirigiendo..."
      );
